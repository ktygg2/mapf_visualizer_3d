...
def calculate_collision_risk_vec(current_pos, other_positions, R):
    # current_pos: (3,), other_positions: (N, 3)
...
for iteration in range(n_replanning):
    ...
    for t_idx, t in enumerate(time_steps):
        current_positions = positions[t_idx]  # (n_agent, 3)
    ...
    # 다음 반복을 위해 positions만 업데이트
    # (이미 positions에 바로 적용 중이므로 별도 업데이트 불필요)

# 최종 결과 저장
result_df = pd.DataFrame(modified_paths)
result_df = result_df.sort_values(['time', 'agent'])
simple_result = result_df[['agent', 'time', 'x', 'y', 'z']]

detailed_output_path = 'neighbor_center_smoothing_detailed.csv'
simple_output_path = 'neighbor_center_smoothing_simple.csv'
result_df.to_csv(detailed_output_path, index=False)
simple_result.to_csv(simple_output_path, index=False)

print(f"\n이동 방향 반대 척력 기반 스무딩 결과를 '{simple_output_path}'에 저장했습니다.")
print(f"상세 결과는 '{detailed_output_path}'에 저장했습니다.")

print("\n힘 적용 통계:")
print(f"평균 인력 크기: {result_df['attractive_magnitude'].mean():.4f}")
print(f"평균 척력 크기: {result_df['repulsive_magnitude'].mean():.4f}")
print(f"평균 충돌 위험도: {result_df['collision_risk'].mean():.4f}")
print(f"평균 변위 크기: {result_df['displacement_magnitude'].mean():.4f}")
